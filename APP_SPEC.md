...
- The host QR panel also has an icon-only copy button that copies the plaintext host offer code.
- The host can scan joiner answer QR codes to complete each connection.
- The host can paste a joiner answer code in the same modal used for answer QR scanning.
...
- The user can paste a host offer code in the same modal used for host QR scanning.
- Because there is no internet signaling server, the joiner must show an answer QR code after scanning the host QR.
- The joiner answer QR panel also has an icon-only copy button that copies the plaintext answer code.
- The host scans the joiner's answer QR code to complete the WebRTC connection.
- The host can paste the copied answer code instead of scanning the answer QR code.
...
- Copy/paste code handoff is a general sync handshake feature, not an online-only feature.
- Nearby/offline sync and future online manual sync should use the same QR/copy/paste handoff UI.
- Every generated QR payload should also be available as a plaintext code.
- Host offer codes use the same compact `QWO:` payload that is rendered in the host QR.
- Join answer codes use the same compact `QWA:` payload that is rendered in the answer QR.
- The copied value should be only the raw payload, such as `QWO:...` or `QWA:...`.
- The UI should not show the long code inline in the normal lobby or answer screen.
- QR panels should show a small icon-only copy button using the standard copy icon: two overlapping squares.
- The host offer QR copy button should have an accessible label such as `Copy host code`.
- The join answer QR copy button should have an accessible label such as `Copy answer code`.
- Copy success should use the existing centered toast style with minimal text, such as `Copied`.
- If clipboard copy fails, the app should open a minimal copy fallback modal.
- The copy fallback modal should show a small textarea containing the raw code, select/focus it when possible, and let the user manually copy.
- The copy fallback should use existing modal styling and should not introduce a visually heavy new flow.

Paste-code intake:

- Pasting a code should be an alternative inside the existing camera modal, not a separate lobby-level workflow.
- The camera modal should show a compact paste section above the camera preview.
- The paste section should use a small, sleek textarea so long or wrapped text can be pasted.
- The paste section should have one concise confirm control, preferably icon/button style consistent with the app.
- The paste section should not add explanatory paragraphs or clutter.
- The same modal should still show the camera scanner below the paste section.
- Pasting and camera scanning must call the same handshake handlers after parsing:
  - Joiner host-code intake calls the same handler as scanning a host QR.
  - Host answer-code intake calls the same handler as scanning an answer QR.
- The app should extract the first valid compact sync code from the pasted text.
- Extraction should accept a raw `QWO:` or `QWA:` code.
- Extraction should also accept surrounding text that contains a valid code, such as a copied text message.
- The extractor should ignore leading/trailing whitespace.
- The extractor should preserve the code exactly after extraction so existing QR parsers handle the payload.
- The modal must know the expected code type:
  - Joiner host intake expects `QWO:`.
  - Host answer intake expects `QWA:`.
- If a pasted code has the wrong compact prefix for the current modal, the app should reject it before attempting the WebRTC handshake.
- Wrong-code messages should be short and specific, such as `Paste a host code` or `Paste an answer code`.
- If no valid compact sync code is found, the app should show a short paste failure message and keep the modal open.
- Legacy `qwixx:` payloads may continue to parse through the existing QR parser for development compatibility, but the visible copy/paste feature is centered on compact `QWO:` and `QWA:` codes.
- The paste textarea content should not be saved after the modal closes.
...
- QR scanning is useful and should remain the primary handshake path, but camera scanning is not always reliable.
- Users should be able to exchange the exact same host offer and join answer payloads by copying and pasting plaintext codes.
- This feature is independent from online sync mode. It should work for the current nearby/offline sync flow and be reusable by future online manual sync.

Required UI changes:

- Add an icon-only copy button to every QR panel.
- Use the standard copy icon with two overlapping squares.
- The host offer QR panel copy button copies the raw `QWO:` code and uses an accessible label like `Copy host code`.
- The join answer QR panel copy button copies the raw `QWA:` code and uses an accessible label like `Copy answer code`.
- Do not display the long code inline in the normal QR panel.
- Copy success uses the existing centered toast style with minimal text, such as `Copied`.
- If clipboard copy fails, open a minimal copy fallback modal.
- The copy fallback modal contains a small textarea with the raw code selected/focused when possible.
- The copy fallback modal should be visually light and should reuse existing modal/button styling.
- The existing host Scan button and join Join/Scan flow should stay visually simple and mostly unchanged.
- Do not add separate lobby-level Paste buttons.
- Add the paste-code alternative inside the existing camera/scanner modal.
- The scanner modal paste section appears above the camera preview.
- The paste section uses a small, sleek textarea.
- The paste section has one compact confirm control.
- The paste section should be visually subordinate to the modal, not a large form.

Required parsing behavior:

- Add one small helper that extracts the first valid compact sync code from pasted text.
- The helper should recognize compact `QWO:` and `QWA:` payloads.
- The helper should trim surrounding whitespace.
- The helper should tolerate surrounding text before or after the code.
- The helper should return the extracted raw code exactly, without reformatting the payload.
- The helper should reject text with no valid compact sync code.
- The scanner modal should know which code type is expected.
- Joiner host-code intake expects `QWO:`.
- Host answer-code intake expects `QWA:`.
- If a pasted compact code has the wrong prefix for the modal, reject it before starting the WebRTC handshake.
- Wrong-prefix messages should be short and specific.
- Pasted host-code intake and scanned host-code intake must call the same `scanHostOffer` path after parsing.
- Pasted answer-code intake and scanned answer-code intake must call the same `acceptJoinAnswer` path after parsing.
- The paste textarea contents should clear when the scanner modal closes.

Required implementation constraints:

- Do not change game rules, turn logic, scoring, Ready flow, row closure, or sync play behavior.
- Do not change the compact QR payload format.
- Do not introduce online sync, STUN, TURN, or a backend in this pass.
- Do not add route/deep-link dependencies for this feature.
- Do not rely on copied codes opening in Safari, Chrome, or the installed PWA.
- Keep QR scanning and paste-code input interchangeable from the app's point of view.
- Reuse existing sync status messages where possible.
- Keep state local and minimal:
  - scanner paste text,
  - scanner paste error/status if needed,
  - copy fallback code if clipboard copy fails.

Required tests:

- Verify host offer QR panel has a copy control with an accessible label.
- Verify join answer QR panel has a copy control with an accessible label.
- Verify copied host code starts with `QWO:`.
- Verify copied answer code starts with `QWA:`.
- Verify the scanner modal renders a paste textarea above the camera preview.
- Verify host-offer paste calls the same joiner answer-creation flow as scanning.
- Verify answer-code paste calls the same host answer-acceptance flow as scanning.
- Verify wrong-prefix paste is rejected before handshake.
- Verify pasted text can contain surrounding message text and still extract the compact code.
- Verify copy fallback exists in source and opens when clipboard copy fails.
- Verify the normal QR scanner still works.
